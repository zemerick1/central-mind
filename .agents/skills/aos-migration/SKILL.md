...
1. Invoke `file_manager` to render the upload widget — and make it the **terminal action of the turn.** Emit the one-line instruction to the operator, call `file_manager`, then **STOP and end your turn.** Do NOT call any further tool (no `skills_load`, `search`, `bash`, `list_files`, or anything else) in the same turn after `file_manager` — wait for the operator to actually drop the file, then resume in a **new** turn at step 2.

   > **Why this matters (render-and-stop):** the upload widget is a live UI component that **re-initializes on every subsequent tool call** in the turn — each later tool call forces the client to re-render and re-mount it, and a chain of trailing calls can leave it stuck on "waiting for content" (never settling enough to accept a drop). Rendering it last and stopping lets it settle. This is a client-side host limitation ([modelcontextprotocol/ext-apps#701](https://github.com/modelcontextprotocol/ext-apps/issues/701)); render-and-stop is the reliable workaround. Operator-facing text to emit: *"Drop or pick your AOS 8 running-config file (the output of `show running-config`, or any file containing `netdestination` / `ip access-list session` / `user-role` stanzas). I'll pick it up once it's uploaded."*
2. **After the operator uploads (a new turn):** the file is held in the server's session store. Call `list_files` to get the uploaded file's **name** (metadata only — name/size/type, no content), then parse it **server-side**: `aos8_parse_config(filename="<uploaded filename>")`. The tool reads the file *inside the server* so the raw config — including PSKs and RADIUS/TACACS secrets — never enters the model context. There is intentionally **no tool that returns the raw uploaded file to you**; do not attempt to read the file contents directly.
